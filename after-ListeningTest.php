...
<?php 
	...
	$query = "select * from answer_listening where Level = '$level' and LevelFT = '$levelFT'";
...
<?php getHeader();?>
	...
	$query = "update scorelisteningtest set level".$level." = $score where email = '$email' ";
	$conn->query($query);
	//write on Database
	if ($score >=4) {
			$level ++;
			$query = "update score set levelListening = '$level'  where email = '$email'";
			$conn->query($query);
	}
    ...
    <div class="content" style='width:80%; margin:unset'>
       <h2 style="text-align: center;font-size: 30px; color: red;margin-bottom: 20px;font-family: 'Montserrat', sans-serif;">Listening Test</h2>
